...
def visualize_explanation(explanation, img_array : np.ndarray, true_label : int, pred_probs : np.ndarray, pred_label : int, save_path : Optional[Path] = None, num_features : int = NUM_FEATURES):
    fig, axes = plt.subplots(1,3, figsize = (15,15))
    axes[0].imshow(img_array)
    axes[0].set_title(f"Original\nTrue : {UNIFIED_CLASSES[true_label]}", fontsize = 12, fontweight = 'bold')
    axes[0].axis("off")
    temp, mask = explanation.get_image_and_mask(pred_label, positive_only = True, num_features = num_features, hide_rest = False)
    ...
    axes[2].set_title(f'All Regions\n'
                      f"Green = For | Red = Against",
                      fontsize = 12)
    axes[2].axis("off")
    fig.suptitle("|".join([f"{UNIFIED_CLASSES[i]} : {pred_probs[i]:.1%}" for i in range(NUM_CLASSES)]),fontsize = 9, y=0.02)
    plt.tight_layout()
    if save_path:
...
def explain_model(
    ...
    for idx, (image, true_label) in enumerate(zip(images, labels)):
        ...
        transform = get_lime_transform()
        tensor = transform(image).unfreeze(0).to(device)
        with torch.no_grad():
            logits = model(tensor)
        ...
        visualize_explanation(explanation, img_array, true_label, pred_label,probs, save_path = save_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--samples", type = int, default = 20, help = "Number of LIME perturbations per image")
